...
def sigma_y1_zero_curve(
    ...
    denom = pi + 2.0 * s * (1.0 - y1)
    num = y1**3 * (1.0 - y1)

    with np.errstate(divide="ignore", invalid="ignore"):
        c_line = (2.0 * s * u * f_line * num) / denom

    bad = (~np.isfinite(c_line)) | (denom <= 0) | (c_line <= 0)
    c_line[bad] = np.nan

    if mask_invasion and f_line.size:
        valid = np.isfinite(c_line)
        if np.any(valid):
...
def pc_and_nmax_levels(s, pi):
    ...
    disc = pi**2 + 2.0 * s * pi
    p_c = (-pi + np.sqrt(disc)) / (2.0 * s)

    if not np.isfinite(p_c) or not (0.0 < p_c < 1.0):
        return np.nan, np.nan
...
def overlay_pc_and_nmax(
    ...
    if np.isfinite(p_c):
        mask_p = np.isfinite(fs) & np.isfinite(cs) & np.isfinite(p_vals)
    ...
    if np.isfinite(n_max):
        mask_n = np.isfinite(fs) & np.isfinite(cs) & np.isfinite(n_vals)
